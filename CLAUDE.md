...
| `simplify-thayto` | Refactors the current branch's diff (vs. `main`) for KISS/DRY with hard structural rules — no classes, no nested ifs, no nested ternaries, prefer small functions — then fixes TypeScript errors/warnings (delegating heavy type work to the `typescript-advanced-types` skill), then runs the test suite. Triggered by "/simplify-thayto", "simplify our changes", "DRY this up", "refactor with KISS", etc. | Edits source files in the working tree of the current checkout and runs the project's test command. Never branches, stages, commits, pushes, or touches any git ref. User reviews the working-tree diff and commits themselves. |
