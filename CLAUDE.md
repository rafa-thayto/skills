...
| `simplify-thayto` | Refactors the current branch's diff (vs. `main`) for KISS/DRY with hard structural rules — no classes, no nested ifs, no nested ternaries, prefer small functions — strips unnecessary comments (keep WHY, remove WHAT / tombstones / ticket references / redundant JSDoc), then fixes TypeScript errors/warnings (delegating heavy type work to the `typescript-advanced-types` skill), then runs the test suite. Triggered by "/simplify-thayto", "simplify our changes", "DRY this up", "refactor with KISS", "remove unnecessary comments", etc. | Edits source files in the working tree of the current checkout (including comment deletions) and runs the project's test command. Never branches, stages, commits, pushes, or touches any git ref. User reviews the working-tree diff and commits themselves. |
| `self-review-thayto` | Combines `pr-review-opinionated-thayto` and `address-my-prs-thayto` for a single PR authored by the user (current branch's PR or a pasted URL): runs the four parallel review agents (security/performance/TypeScript/code-quality) over the PR diff, then applies the apply/escalate/skip rubric to act on each finding locally. Findings are never posted to GitHub. Triggered by "/self-review-thayto", "self-review my PR", "pre-flight my PR", "review my PR myself first", etc. | Read-only on GitHub (no review submission, no comments, no thread resolution). Edits source files in the working tree of the current checkout to apply rubric-cleared findings; never stages, commits, pushes, rebases, or touches any git ref. User reviews the working-tree diff and commits themselves. Refuses if the PR's author is not the authenticated user. |
